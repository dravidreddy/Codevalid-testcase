...
CASE_SUFFIX="$(date +%s)-$$"
RESPONSE_BODY="/tmp/input_validation_empty_message_response_${CASE_SUFFIX}.txt"
RESPONSE_HEADERS="/tmp/input_validation_empty_message_headers_${CASE_SUFFIX}.txt"
cleanup_files(){ rm -f "$RESPONSE_BODY" "$RESPONSE_HEADERS"; }
trap cleanup_files EXIT
# Given

echo "STEP: Given — chatbot API is reachable"
echo "PREREQ: no setup required for validation request"
# When

echo "STEP: When — send empty message"
...
echo "STEP: Then — API rejects empty message"
...
echo "CODEVALID_TEST_ASSERTION_OK:input_validation_empty_message"
