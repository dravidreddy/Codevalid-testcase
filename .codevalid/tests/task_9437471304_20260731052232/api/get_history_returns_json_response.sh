...
RESPONSE_BODY_FILE="/tmp/get_history_returns_json_response_body_${CASE_SUFFIX}.txt"
RESPONSE_HEADERS_FILE="/tmp/get_history_returns_json_response_headers_${CASE_SUFFIX}.txt"
CLEAR_BODY_FILE="/tmp/get_history_returns_json_response_clear_body_${CASE_SUFFIX}.txt"
CLEAR_HEADERS_FILE="/tmp/get_history_returns_json_response_clear_headers_${CASE_SUFFIX}.txt"
...
echo "STEP: Given — ensure API is in a clean state"
echo "PREREQ: clearing chat history"
...
echo "STEP: When — request history endpoint response"
...
echo "STEP: Then — verify JSON formatted response"
...
grep -F '"history":' "$RESPONSE_BODY_FILE" || { echo "ASSERTION_FAILED: expected history field in JSON body"; exit 1; }
...
echo "CODEVALID_TEST_ASSERTION_OK:get_history_returns_json_response"
