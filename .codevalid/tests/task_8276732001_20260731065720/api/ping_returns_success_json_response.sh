...
RESPONSE_HEADERS="/tmp/ping_returns_success_json_response_headers_${CASE_SUFFIX}.txt"
RESPONSE_BODY="/tmp/ping_returns_success_json_response_body_${CASE_SUFFIX}.txt"
...
# Then
echo "STEP: Then — verify successful JSON response"
[ "$HTTP_STATUS" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${HTTP_STATUS}"; exit 1; }

grep -i 'content-type: application/json' "$RESPONSE_HEADERS" >/dev/null 2>&1 || {
  echo "ASSERTION_FAILED: expected Content-Type application/json"
  exit 1
}
...
echo "CODEVALID_TEST_ASSERTION_OK:ping_returns_success_json_response"
